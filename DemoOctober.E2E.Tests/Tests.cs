...
namespace DemoOctober.Module.E2E.Tests;

public class DemoOctoberTests : IDisposable {
    const string BlazorAppName = "DemoOctoberBlazor";
    const string WinAppName = "DemoOctoberWin";
    const string AppDBName = "DemoOctober";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public DemoOctoberTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\DemoOctober.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\DemoOctober.Win\bin\EasyTest\net6.0-windows\DemoOctober.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "DemoOctoberEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
