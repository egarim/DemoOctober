...
using DemoOctober.WebApi.JWT;
using DevExpress.ExpressApp.Security.Authentication;
using DevExpress.ExpressApp.Security.Authentication.ClientServer;
using DemoOctober.WebApi.Core;
using DevExpress.ExpressApp.AspNetCore.WebApi;

namespace DemoOctober.WebApi;
...
public class Startup {
    ...
    public void ConfigureServices(IServiceCollection services) {
        ...
        services.AddXafAspNetCoreSecurity(Configuration, options => {
            ...
            options.UserType = typeof(DemoOctober.Module.BusinessObjects.ApplicationUser);
            // ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
            // If you use PermissionPolicyUser or a custom user type, comment out the following line:
            options.UserLoginInfoType = typeof(DemoOctober.Module.BusinessObjects.ApplicationUserLoginInfo);
        ...
        services.AddSwaggerGen(c => {
            ...
            c.SwaggerDoc("v1", new OpenApiInfo {
                Title = "DemoOctober API",
                Version = "v1",
                Description = @"Use AddXafWebApi(options) in the DemoOctober.WebApi\Startup.cs file to make Business Objects available in the Web API."
            });
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
        if(env.IsDevelopment()) {
            ...
            app.UseSwagger();
            app.UseSwaggerUI(c => {
                c.SwaggerEndpoint("/swagger/v1/swagger.json", "DemoOctober WebApi v1");
            });
        }
        else {
